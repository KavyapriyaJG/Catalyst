...
def generate_prd(state: AgentState):
    ...
    if source == "code_only":
        ...
        print("   Using COBOL-specific PRD generator (code-only flow)")
    elif source == "documents_only":
        prompt_template = DOCUMENT_PRD_GENERATOR_PROMPT
        print("   Using document-specific PRD generator (documents-only flow)")
    elif source == "code_and_documents":
        prompt_template = COMBINED_PRD_GENERATOR_PROMPT
        print("   Using combined PRD generator (code + documents flow)")
    else:
        raise ValueError(
            f"Unknown analysis source: {source}. "
            "Expected 'code_only', 'documents_only', or 'code_and_documents'"
        )
    ...
    try:
        ...
        print(
            f"   Codex responded in {int(elapsed)}s — "
            f"PRD generated (JSON with {len(prd_json)} top-level fields)"
        )
        return {"prd": prd_json}
    except ValueError as e:
        print(f"   Failed to parse PRD JSON: {e}")
        return {"prd": _EMPTY_PRD}
