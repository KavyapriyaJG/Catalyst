...
from prd_generation.output_formatter import extract_json_from_response
...
def reconcile(state: AgentState):
    ...
    prd_json_str = (
        json.dumps(state["prd"], indent=2) if isinstance(state["prd"], dict) else str(state["prd"])
    )
    analysis_str = (
        json.dumps(state["analysis"], indent=2)
        if isinstance(state["analysis"], dict)
        else str(state["analysis"])
    )
    ...
    try:
        ...
        print(
            ...
            f"Reconciled PRD (JSON with {len(prd_json)} top-level fields)"
        )
